...
function setEditarCadastrarProdutoDialog(produto: Produto | null) {
  $q.dialog({
    ...
    componentProps: {
      produto: produto,
      subModulo: 'AQUATICO',
      labelHeader: 'material de salvamento aquático'
    },
  }).onOk(()=>{
    load()
  })
...
async function onDeleteProduto(_id: string){
  $q.dialog({
    ...
    componentProps: {
      bodyText: 'Você tem certeza que deseja excluir o material?',
...
async function load(){
  try {
    loading.value = true
    const data = await services.produtosService.listProdutos('AQUATICO')
    produtos.value = data
  }finally{
    loading.value = false
  }
